...
ok(/'v4:' \+ contractFormHashV4_/.test(f29),'個別条件スナップショットをv4 hash化');
ok(/法人申込は現行制度の標準契約対象外/.test(f29),'法人は標準自動締結外');
ok(/イベント利用は制度条件の個別確認/.test(f29),'イベントは個別確認');
ok(/terms_snapshot_hash/.test(f36) && /contractFormHashV4_\(received\)/.test(f36),'FormRun受信時に個別条件改変を検知');
ok(/function snapshotContractTerms_\(/.test(f33) && /contract_template_version/.test(f29),'締結時スナップショットをv4個別条件化');
ok(/web_createApplicationV4/.test(patch),'ポータルがv4申込APIを使用');
ok(/ガイドライン/.test(patch) && /CloudSign上で/.test(patch),'ポータル表示がCloudSign締結点を明示');
ok(/29_contract_form_v4\.gs/.test(build) && /36_formrun_contract_v4\.gs/.test(build),'ビルドにv4モジュールを含む');
