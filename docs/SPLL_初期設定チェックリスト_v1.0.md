...
| **Config（SS_OPSのConfigシート）** | 運用値（振込先・フォームURL・文面など） | 管理画面「設定」 | **3プロジェクト共通・即時反映**（再デプロイ不要） |
...
**振込先（利用許諾料）** セクションの「利用者向けページURL」に **workflow の `/exec`** を入れます（`WORKFLOW_URL`）。
...
| 項目 | Configキー | 例 |
|---|---|---|
| 銀行名 | `PAYMENT_BANK_NAME` | ○○銀行 |
| 支店名 | `PAYMENT_BRANCH` | ○○支店 |
| 預金種別 | `PAYMENT_ACCOUNT_TYPE` | 普通 |
| 口座番号 | `PAYMENT_ACCOUNT_NUMBER` | 1234567 |
| 口座名義 | `PAYMENT_ACCOUNT_HOLDER` | 株式会社○○ |
| 名義カナ | `PAYMENT_HOLDER_KANA` | カ）○○ |
| 備考 | `PAYMENT_NOTE` | 振込手数料はご負担ください／お振込名義にSPLL番号を… |
...
口座情報は**案内ページにのみ**掲載されます（メール本文には書きません）。変更は即時反映です。
...
- [ ] 5分以内に案内メールが届き、案内ページで **SPLL番号・振込先・提出リンク・バッジ** が見える
