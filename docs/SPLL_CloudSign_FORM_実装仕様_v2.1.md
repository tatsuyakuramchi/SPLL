...
**成年確認の扱い（2026-08-27 変更）**
自己申告のチェックボックスから生年月日へ変更した。受信時に `adultCheckFromBirthDate_` が
JSTの暦日で年齢を数え、判定結果だけを `Applications.adult_check`（ADULT/MINOR/UNKNOWN）へ記録する。
**生年月日そのものは台帳へ保存しない**（保有する個人情報を増やさないため。事務局が必要とするのは可否と理由だけ）。

ADULT 以外は `manual_review_reason` を立てて事務局へ通知し、締結後の条件照合（§10.8）でも
自動有効化を止める。解釈できない値・未取得も UNKNOWN として成年とみなさない。未成年との契約は
取り消される可能性があるため、判定できないまま自動で進めない（フェイルクローズ）。

したがって **`birth_date` を `FORMRUN_FIELD_MAP_FIXED` / `_RATE` に登録しないと、
全申込が UNKNOWN になり個別確認へ回る**。フォーム作成時に必ず対応付けること。
