...
期待：`ok` が返り、
- [ ] admin「ライセンス」で案件が **作品提出待ち（AWAITING_SUBMISSION）／契約=締結済／認証=未発行**
- [ ] 台帳 Contracts の `terms_verification_status=VERIFIED`
- [ ] Access_Tokens（SUBMISSION・GUIDE）が発行され、**Certificates・Badges はまだ無い**（認証は審査完了後・RP-002）
- [ ] License_Cases の `case_status=AWAITING_SUBMISSION`／Finance_Handoffs に READY の引渡行（経理側が参照するスナップショット）
- [ ] Drive に `SPLL-番号` 名の案件フォルダができている
...
1. admin「設定→手続き案内」で `WORKFLOW_URL`（Cloud Run のURL）を設定
2. admin「ライセンス」→ 対象案件の「案内リンク」→ 表示されたURLを開く
   - [ ] SPLL番号・契約者・対象原作・利用許諾料・クレジット表記・審査状況が出る
   - [ ] **振込先は出ない**（契約書に記載。ページには「お振込先は契約書に記載」の注記だけ）
   - [ ] 「作品提出ページを開く」で提出ページが別タブで開く
   - [ ] 認証バッジは「作品のご提出と審査の完了後に発行されます」と表示される（この時点では未発行）
3. 「案内リンク」を再発行 → 以前のURLを開くとエラーになる

### 2.3.2 自動メールの送信

1. admin「設定→手続き案内→自動メールの送信」で、差出人名・返信先を設定し、
   種類を選んで「テスト送信」で自分宛に見本を受信（4種類とも確認する）
   - [ ] 締結直後のご案内：件名にSPLL番号、本文に案内ページURL
   - [ ] 作品の審査完了／認証バッジの発行完了：本文に案内ページURL
   - [ ] **どの本文にも口座情報が入っていない**（振込先は契約書のみ）
...
> 審査完了・バッジ発行のメールはフェーズ3（作品提出・審査）で届きます。§4 で確認します。
> 停止したい場合は同画面の各チェックを外します（`GUIDE_EMAIL_AUTO_SEND` ほか）。
> 送信失敗は3回まで自動再試行し、超えると `SEND_FAILED` として要対応一覧に理由つきで残ります。

### 2.3.3 認証の一時停止／復帰（利用許諾料の入金が確認できないとき）

> 認証は作品の審査完了（フェーズ3）で発行されます。**フェーズ3を終えてから**実施してください。
> 請求・入金の管理は SPLL の外（経理）で行います。ここでは認証の表示を止める／戻すだけです。

1. admin「認証管理」→ 対象の「**認証を停止（利用許諾料未確認）**」→ 理由を入力
   - [ ] 認証が「停止」表示になり、理由欄に入力した理由が出る（状態=SUSPENDED、理由コード=FEE_PAYMENT_UNCONFIRMED）
   - [ ] ライセンスタブの現在地が「認証停止（SUSPENDED）」になる
   - [ ] バッジQR（または検証ページ）で照会すると「**無効**」と表示される
   - [ ] 提出ページ・案内ページからバッジをダウンロードできない（停止中は配布しない）
2. 「**認証を復帰（入金確認）**」で戻す
   - [ ] 検証が「確認済み」に戻り、現在地が「認証済（CERTIFIED）」に戻る
   - [ ] バッジを再びダウンロードできる

> 失効（REVOKED）・契約終了（TERMINATED）や、法務判断など他の理由で止めた停止からの復帰は、申請→別担当者の承認が必要です。
> 契約が終了した案件の認証は再有効化できません（再開は新しい契約の締結）。

### 2.4 条件不一致（TERMS_MISMATCH）の確認

2.1でもう1件申込を作成し、**2.2をスキップして** 2.3の締結Webhookだけ送る（document_idは別の値に）。

期待：応答 `accepted-manual-review`。契約は作成されるが提出導線は開かず、ライセンスの現在地は「条件確認中（HOLD）」。
admin「ライセンス」タブ下段の「条件不一致（TERMS_MISMATCH）」に表示 → 「条件確認済み（有効化）」で提出導線・経理引渡が動き、現在地が「作品提出待ち」へ。
...
期待：「ライセンス」タブ下段の「メール不達」に表示 → 「対応済みにする」で消える。

---

## フェーズ3：提出〜審査（15分）

1. admin「ライセンス」→ 対象案件の「提出リンク」→ URLを開く（SPLL番号が表示される）
2. PDFを提出 → AI一次チェック。ライセンスの現在地が「審査中（REVIEWING）」になる
   - GCP/Gemini設定済み：AI_SCREENED → 審査キューへ
   - 未設定：自動再試行の後 **AI_UNAVAILABLE → 人手審査へ回送**（これも正常系）
3. admin「作品審査」→ **是正要求** → 現在地が「是正対応中」、是正コメントが提出ページに出る。認証はまだ無い
4. 再提出 → admin「作品審査」→ **承認（CLEARED）** → トーストに「認証を発行しました」
   - [ ] admin「認証管理」に認証が **有効** で載り、バッジが「発行済」
   - [ ] ライセンスの現在地が「認証済（CERTIFIED）」
   - [ ] 提出ページ・案内ページに「認証バッジをダウンロード」が出て、QR入りPNGが取得できる
   - [ ] ライセンス詳細の「履歴」に 申込受付→契約成立→作品提出→是正要求→審査 承認→認証発行 が並ぶ
5. 検証ポータル：照合コード再発行（rotate）→ 表示されたURLで「確認済み」表示（SPLL番号・クレジット表記が出る）
6. 未解決の重大アラート（HIGH_RISK）が残る案件を承認しても認証は発行されず、トーストに理由が出る（アラートを閉じて再承認）
7. 承認から5分待つ（またはworkflowで `trigger_every5min` を手動実行）
   - [ ] 契約者へ **審査完了** のメールが届く（本文に案内ページのリンク）
   - [ ] 契約者へ **認証バッジの発行完了** のメールが届く（本文に案内ページのリンク）
   - [ ] どちらのリンクも**締結時に届いた案内メールと同じURL**（毎回変わらない）
   - [ ] 是正要求（手順3）では自動送信されず、要対応一覧に `CORRECTION_REQUEST` が残る
     （admin「設定→手続き案内→自動メールの送信」で有効にすると、審査コメント入りで自動送信されます）
   - [ ] **上申（ESCALATED）** にした案件では審査完了メールが送られない（要対応一覧に残る）
...
1. admin「設定→AI審査」→ プロンプトを編集 →「プレビュー」で送信文面（ルール差込後）を確認
2. 「プロンプトを保存」→ 適用中の版が変わることを確認
3. 作品を1件提出 → `AI_Review_Jobs` の `prompt_version` が保存後の版になっていること
4. 「既定文を読み込む」→ 保存 → 既定へ戻せること

> `{{rules}}` を消して保存すると警告が出ますが、ルールは本文末尾へ自動付与されるため審査条件は欠落しません。
