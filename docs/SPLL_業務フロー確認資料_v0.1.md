...
**版** v0.3（ドラフト） / **対象** 現行運用フローとのすり合わせ / **作成** 法務部
...
>
> **v0.3 主な変更点（修正設計書 SPLL-SYS-RD-001 反映）**：管理機能に**ロール別権限（RBAC）**を導入し、
> Webhookは**受信記録＋真正性検証**（CloudSignはAPI照会必須）、提出は**サーバー側ファイル検証**、
> アクセスは**用途別トークン**（期限・回数付き）に統一。利用報告に**承認フロー**、請求は**自動起票**
> （定額系=締結時／売上連動=報告承認後）。GASは**公開ポータル／業務処理／管理コンソールの3プロジェクトに分離**。
> ※ v0.2: B経路固定・複数原作・application_ref突合・版管理・締結時認証発行。
>
...
| 事務局・審査／法務 | 人手 | AI結果を受けた最終判断・是正要求・エスカレーション（**ロール別権限**：LEGAL_ADMIN／OPERATIONS） |
| 事務局・経理 | 人手 | 利用報告の承認・請求・入金消込・半期清算の承認（ロール：ACCOUNTING） |
...
| 5 | 利用者 | 締結 | 手 | — |  |
| 6 | CloudSign→GAS | 締結通知（**application_ref突合**）→契約＋対象原作固定＋認証＋提出トークン | 自動 | Contracts(SIGNED) / Contract_Works / Certificates(ACTIVE) / Access_Tokens |  |
...
>
> **v0.3 セキュリティ強化**：Webhookは共有秘密＋**受信記録（Webhook_Receipts）**で管理し、本番では
> **CloudSign API照会で締結完了を確認してから**契約を登録します（受信データだけでは契約を作らない）。
> 締結と同時に**締結済PDF原本をハッシュ付きで保存**し、突合できない締結は「未紐付け」として保留
> →管理コンソールで手動紐付けします。申込時は公開中の原作・有効な利用目的かをサーバー側で検証し、
> **同意文・規約のハッシュ**を申込に記録します（同意証跡）。

---

## 4. 作品提出・審査（締結後・版管理・複数原作）

締結後、利用者は**専用の提出ページ**（用途別トークン Access_Tokens）から作品を提出します。Google Driveの共有フォルダを直接開放しません。
...
> **v0.3 強化**：アップロードは**サーバー側で検証**（PDF/PNG/JPEGのみ・20MB・拡張子/MIME/先頭シグネチャ一致・
> 提出回数の上限管理）。是正要求のコメントは**提出ページに表示**され、利用者は内容を見て修正版を再提出できます。
> 人手審査は**是正・上申にコメント必須**、**最新版以外への誤審査を拒否**、審査者はログインユーザーから自動記録します。
...
| # | レーン | 作業 | 自動/人手 | 記録先 | 現行運用／確認・修正 |
|---|---|---|---|---|---|
| 1 | 事務局 | 報告依頼（トークンURL共有） | 手/自動 | Access_Tokens(REPORT) |  |
...
> **v0.3 追加**：利用報告は **SUBMITTED →（差戻し RETURNED ⇄ 再提出）→ APPROVED → LOCKED** の承認フローで管理し、
> 差戻しには理由を必須とします。請求は**自動起票**：定額（FLAT）・原作数比例（PER_WORK）は**締結時**、
> 売上連動（RATE）は**報告の承認後**に管理コンソールから起票します（無償は起票せず記録のみ）。
...
- **v0.3**：照合コードは**12桁の暗号学的乱数**とし、台帳には**ハッシュのみ**保存（総当たり・漏えい対策）。照会の不一致はログに記録。
- **要確認**：失効の運用ルール（未入金時に `PAYMENT_HOLD` にするか 等）。
...
- **複数原作の配分**：配分方式はマスタ設定で明示（**既定＝BY_WORK_EQUAL：対象原作数で均等・同一権利者は合算**／BY_PARTNER_EQUAL選択可）。明細に原作・権利者・方式・比率を記録。この既定で妥当か
...
| 清算 | 異議対応・配分支払 | 経理 | 半期 |  |
| 報告 | 利用報告の確認・差戻し・承認・ロック | 経理 | 随時 |  |
| 請求 | RATE請求の起票（承認済み報告から） | 経理 | 期次 |  |
| マスタ | 原作マスタ・料率・同意文/規約の更新 | 事務局/法務 | 随時 |  |
| 権限 | 管理者・ロールの登録・無効化（RBAC） | システム管理者 | 随時 |  |
...
| Usage_Reports | SUBMITTED →（RETURNED ⇄ 再提出）→ APPROVED → LOCKED（/ SUPERSEDED） |
| Invoices | 入金待ち → 入金済（/ 取消）※起票元を記録（CONTRACT=締結時定額系／REPORT=RATE承認後） |
...
| Webhook_Receipts | RECEIVED → PROCESSED / ERROR（再試行） / REJECTED（検証不一致） |
| Admin_Users（RBAC） | SYSTEM_ADMIN / LEGAL_ADMIN / OPERATIONS / ACCOUNTING / AUDITOR（ACTIVE/DISABLED） |
...
- **GASは3プロジェクトに分離**（公開ポータル／業務処理／管理コンソール=組織内限定）。管理機能は
  ロール別権限（RBAC）でサーバー側強制。詳細は「SPLL_3プロジェクト分割手順_v1.0」参照。
- **環境切替**：`ENVIRONMENT=production` で全フェイルクローズ（Webhook検証必須・匿名管理拒否・
  サンプル表示なし）が有効。試験中は development のまま運用。
