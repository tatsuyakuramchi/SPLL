...
> **★＝今回の追加業務**：①作品マスタ更新→**X（Twitter）告知投稿**、②入金確認後→**認証バッジ（クレジット表記）の発行・配布**。
...
作品マスタを更新し公開（PUBLISHED）にした時点で、X（Twitter）へ告知投稿します（集客・認知）。
...
sequenceDiagram
  ...
  OF->>DB: 作品を登録・更新し「公開」にする
  alt 自動投稿ON
    SYS->>X: 告知を自動投稿（テンプレ文面）
  else 手動
    OF->>SYS: 管理画面から「Xへ投稿」
    SYS->>X: 告知を投稿
  end
  Note over SYS,X: 同一作品は二重投稿しない（1回のみ）
```

| # | レーン | 作業 | 自動/人手 | 記録先 | 現行運用／確認・修正 |
|---|---|---|---|---|---|
| 1 | 事務局 | 作品マスタ更新・公開 | 手 | Works_Master(PUBLISHED) |  |
| 2 | システム | 公開時にX告知（自動）／管理画面から手動 | 自動/手 | Events / 投稿済フラグ |  |

**確認したい運用ルール**：自動投稿ON/OFF・**投稿文面のテンプレート**・投稿するX**アカウント**・投稿タイミング。
...
入金確認（入金済）の後に、**認証バッジ**（クレジット表記）を PNG 3サイズ（大/中/小）で自動生成し、
利用者へ配布します（正規ライセンスの証）。
...
sequenceDiagram
  ...
  actor U as 利用者
  OF->>DB: 入金を記録（入金済）
  SYS->>SYS: バッジを組版（作品名・ライセンスID・発行日・クレジット）
  SYS->>DR: PNG 3サイズ（大/中/小）を保存
  SYS->>DB: Badges に発行記録・DLトークン
  SYS-->>U: DLページのURLをメール通知
  U->>DR: バッジPNGをダウンロード
```

| # | レーン | 作業 | 自動/人手 | 記録先 | 現行運用／確認・修正 |
|---|---|---|---|---|---|
| 1 | 事務局 | 入金記録（入金済） | 手 | Payments / Invoices |  |
| 2 | システム | バッジPNG3サイズを生成・Drive保存 | 自動 | Badges / Drive |  |
| 3 | システム | DLページURLを利用者へメール通知 | 自動 | Events |  |
| 4 | 利用者 | バッジをダウンロード・クレジット表記に利用 | 手 | — |  |

**確認したい運用ルール**
- **発行トリガー**：入金確認で自動発行（既定）でよいか（締結時／手動発行にするか）
...
| 告知 ★ | X投稿の文面確認・手動投稿（手動運用時） | 事務局/広報 | 随時 |  |
...
| C-13 ★ | X告知 | 投稿アカウント・自動/手動・文面テンプレ・投稿タイミング |  | 未 |
| C-14 ★ | 認証バッジ | 発行トリガー・デザイン/掲載情報・3サイズ用途・使用条件 |  | 未 |
