...
| 項目 | 内容 |
|
> **⚠️ 本設計書は廃止されました（2026-08-12）**
> 経理は本システムの外で独自の運用を構築する方針となったため、本設計書の実装
> （経理連携GAS④・原票取込・突合・配分・銀行照合・出力・経理ジョブ、および
> License側の利用報告・請求・入金・清算機能）は**リポジトリから全削除**されました。
> SPLLシステムは契約管理（作品の認証管理・バッジ・検証を含む）専用です。
> クラウドサイン運用拡張（§10：送信状態管理・条件照合・不達・訂正・例外対応）のみ
> 契約管理機能の一部として存続しています（管理コンソール「契約管理」タブ）。
